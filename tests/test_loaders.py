...
def test_loadDicomSITK(nsclcCTPath):
    ...
    assert actual.GetSpacing() == (
        0.693359375,
        0.693359375,
        1.0,
    ), "Wrong spacing"
    assert actual.GetOrigin() == (
        -182.1533203125,
        -314.1533203125,
        -305.0,
    ), "Wrong origin"
...
def test_loadSegmentationSEG(nsclcSEGPath):
    ...
    actual = loadSegmentation(segImagePath=nsclcSEGPath, modality="SEG")

    assert isinstance(actual, dict), "Wrong object type, should be dictionary"
    ...
    assert isinstance(actualImage, sitk.Image), "Wrong object type"
    assert actualImage.GetSize() == (512, 512, 304, 1), "Wrong image size"
    assert actualImage.GetSpacing() == (
        0.693359375,
        0.693359375,
        1.0,
        1.0,
    ), "Wrong spacing"
    assert actualImage.GetOrigin() == (
        -182.1533203125,
        -314.1533203125,
        -305.0,
        0.0,
...
def test_loadSegmentationRTSTRUCT(lung4DRTSTRUCTPath, lung4DCTPath):
    ...
    actual = loadSegmentation(
        segImagePath=lung4DRTSTRUCTPath,
        modality="RTSTRUCT",
    ...
    assert isinstance(actualImage, sitk.Image), "Wrong object type"
    assert actualImage.GetSize() == (512, 512, 99), "Wrong image size"
    assert actualImage.GetSpacing() == (0.9766, 0.9766, 3.0), "Wrong spacing"
    assert actualImage.GetOrigin() == (
        -250.0,
        -163.019,
        -1132.0,
