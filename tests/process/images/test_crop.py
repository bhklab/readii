...
from readii.image_processing import loadSegmentation
...
def lung4D_image(lung4D_ct_path):
    return sitk.ReadImage(lung4D_ct_path)

@pytest.fixture
def lung4D_mask(lung4D_ct_path, lung4D_rt_path):
    segDictionary = loadSegmentation(lung4D_rt_path, modality = 'RTSTRUCT',
                                     baseImageDirPath = lung4D_ct_path, roiNames = 'Tumor_c.*')
...
def test_crop_image_to_mask_methods(lung4d_image, lung4D_mask, crop_method, expected_size, request):
    """Test cropping image to mask with different methods"""
    cropped_image, cropped_mask = crop_image_to_mask(lung4d_image, lung4D_mask, crop_method, resize_dimensions=request.getfixturevalue(crop_method))
