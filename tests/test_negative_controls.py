...
@pytest.mark.parametrize(
    "control, expected_first_voxel, expected_last_voxel, expected_central_voxel",
    [
        ("shuffled.full", -987, 10, -118),
        ("randomized.full", 2156, 90, -840),
        ("randomized_sampled.full", -1005, 414, 49),
        ("shuffled.roi", -740, -740, -322),
        ("randomized.roi", -740, -740, 1),
        ("randomized_sampled.roi", -740, -740, -81),
        ("shuffled.non_roi", -840, -490, -1),
        ("randomized.non_roi", -124, 123, -1),
        ("randomized_sampled.non_roi", -653, -805, -1)
    ]
)
def test_apply_negative_control(
    nsclcCTImage: sitk.Image, 
    nsclcSEGImage: sitk.Image, 
    randomSeed: Literal[10], 
    control, 
    expected_first_voxel, 
    expected_last_voxel, 
    expected_central_voxel,
    ):
    "Dynamically test various negative controls by applying them and checking the results"
    # Retrieve the appropriate control using the Factory
    ...
    # Basic assertions
    assert nsclcCTImage.GetSize() == processed_image.GetSize(), \
        f"{control_type} image size not same as input image"
    assert nsclcCTImage.GetSpacing() == processed_image.GetSpacing(), \
        f"{control_type} image spacing not same as input image"
    assert nsclcCTImage.GetOrigin() == processed_image.GetOrigin(), \
        f"{control_type} image origin not same as input image"
