...
def saveCorrelationHeatmap(
	...
	"""
	# Set up the writer
	corr_heatmap_writer = PlotWriter(
		root_directory=correlation_directory,
	...
	# Save the heatmap
	try:
		return corr_heatmap_writer.save(
			plot_figure,
			ColorMap=cmap,
		...
		return corr_heatmap_writer.resolve_path(
			ColorMap=cmap, FeaturesPlotted=feature_type_str, CorrelationType=correlation_type
		)


def saveCorrelationHistogram(
	plot_figure: Figure,
	correlation_directory: Path,
	feature_types: list[str],
	correlation_type: str,
	...
	"""
	corr_histogram_writer = PlotWriter(
		root_directory=correlation_directory,
	...
	# Save the heatmap
	try:
		return corr_histogram_writer.save(
			plot_figure, FeaturesPlotted=feature_type_str, CorrelationType=correlation_type
		)
	except PlotWriterPlotExistsError as e:
		logger.warning(e)

		# If plot file already exists, return the path to the existing plot
		return corr_histogram_writer.resolve_path(
			FeaturesPlotted=feature_type_str, CorrelationType=correlation_type
		)
...
def plotCorrelationHeatmap(
	...
	if diagonal:
		...
		if triangle == "lower":
			...
			msg = (
				f"If diagonal is True, triangle must be either 'lower' or 'upper'. Got {triangle}."
			)
	...
	try:
		corr_ax = sns.heatmap(correlation_matrix_df, mask=mask, cmap=cmap, vmin=-1.0, vmax=1.0)
...
def plotSelfCorrHeatmap(
	...
	self_corr_heatmap = plotCorrelationHeatmap(
		self_corr,
		diagonal=True,
		cmap=cmap,
	...
	if save_dir_path is not None:
		...
		self_corr_save_path = saveCorrelationHeatmap(
			self_corr_heatmap,
			save_dir_path,
			cmap=cmap,
...
def plotCrossCorrHeatmap(
	...
	cross_corr = getCrossCorrelations(
		correlation_matrix, vertical_feature_name, horizontal_feature_name
	)

	# Make the heatmap figure
	cross_corr_heatmap = plotCorrelationHeatmap(
		cross_corr,
		diagonal=False,
		cmap=cmap,
	...
	if save_dir_path is not None:
		# Save the heatmap to a png file
		cross_corr_save_path = saveCorrelationHeatmap(
			cross_corr_heatmap,
			save_dir_path,
			cmap=cmap,
...
def plotSelfCorrHistogram(
	...
	self_corr_hist, _, _ = plotCorrelationHistogram(
		self_corr,
		num_bins=num_bins,
	...
	if save_dir_path is not None:
		...
		self_corr_save_path = saveCorrelationHistogram(
			self_corr_hist,
			save_dir_path,
			feature_types=[feature_type_name],
			correlation_type=f"{correlation_method}_self",
...
def plotCrossCorrHistogram(
	...
	# Make the histogram figure
	cross_corr_hist, _, _ = plotCorrelationHistogram(
		cross_corr,
		num_bins=num_bins,
	...
	if save_dir_path is not None:
		# Save the histogram to a png file
		cross_corr_save_path = saveCorrelationHistogram(
			cross_corr_hist,
			save_dir_path,
			feature_types=[vertical_feature_name, horizontal_feature_name],
			correlation_type=f"{correlation_method}_cross",
