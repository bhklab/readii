...
def getFeatureCorrelations(
	...
	features_to_correlate = vertical_features.join(
		horizontal_features,
		how="inner",
...
def getSelfCorrelations(correlation_matrix: pd.DataFrame, feature_type_name: str) -> pd.DataFrame:
	...
	self_correlations = correlation_matrix.filter(like=feature_type_name, axis=0).filter(
		like=feature_type_name, axis=1
	)
...
def getCrossCorrelations(
	...
	cross_correlations = correlation_matrix.filter(like=vertical_feature_name, axis=0).filter(
		like=horizontal_feature_name, axis=1
	)
...
def getSelfAndCrossCorrelations(
	correlation_matrix: pd.DataFrame,
	vertical_feature_name: str = "_vertical",
	horizontal_feature_name: str = "_horizontal",
) -> pd.DataFrame:
	...
	"""
	try:
		vertical_correlations, horizontal_correlations = (
			getSelfCorrelations(correlation_matrix, vertical_feature_name),
			getSelfCorrelations(correlation_matrix, horizontal_feature_name),
		)
	...
	try:
		cross_correlations = getCrossCorrelations(
