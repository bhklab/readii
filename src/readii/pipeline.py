...
def main() -> None:
    ...
    radFeatOutPath = os.path.join(
        outputDir, 'features/', 'radiomicfeatures_original_' + datasetName + '.csv'
    )
    ...
    if args.negative_controls is not None:
        ...
        for negativeControl in negativeControlList:
            ncRadFeatOutPath = os.path.join(
                outputDir,
                'features/',
                'radiomicfeatures_' + negativeControl + '_' + datasetName + '.csv',
            )
