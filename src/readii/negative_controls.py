...
def getArrayFromImageOrArray(imageOrArray: Union[Image, ndarray]) -> ndarray:
    ...
    """
    assert isinstance(imageOrArray, (Image, ndarray)), \
...
def makeRandomImage(
    ...
    """Generate random pixel values based on the range of values in a sitk.Image or np.ndarray (developed for 3D, should work on 2D as well)
...
def makeRandomSampleFromDistributionImage(
    ...
    """Randomly sample all the pixel values the distribution of existing values in a sitk.Image or np.ndarray.
...
def negativeControlROIOnly(
    ...
    if negativeControlType not in ["shuffled", "randomized", "randomized_sampled"]:
        msg = "negativeControlType must be one of 'shuffled', 'randomized', or 'randomized_sampled'"
    ...
    if binROIMask.any() == False:
        msg = "ROI mask is all 0s. No pixels in ROI to apply negative control to. ROI pixels should be > 1."
        raise ValueError(msg)
