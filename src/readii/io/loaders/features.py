...
def loadFeatureFilesFromImageTypes(
	extracted_feature_dir: Union[Path | str],  # noqa
	image_types: list,
	drop_labels: Optional[bool] = True,
	labels_to_drop: Optional[list] = None,
	...
	for image_type in image_types:
		try:
			...
			match len(matching_files):
				...
				case 0:
					...
					logger.warning(
						f"No {image_type} feature csv files found in {extracted_feature_dir}"
					)
			...
			logger.warning(
				f"Error loading {image_type} feature csv files from {extracted_feature_dir}: {e}"
			)
			raise e
		...
		try:
			...
			logger.warning(
				f"{feature_file_path} does not have the labels {labels_to_drop} to drop."
			)
			# Skip to the next image type
			continue
