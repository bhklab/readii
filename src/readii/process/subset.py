...
def dropUpToFeature(
	dataframe: DataFrame, feature_name: str, keep_feature_name_column: Optional[bool] = False
) -> DataFrame:
	...
	try:
		if keep_feature_name_column:
			...
			column_names = dataframe.columns.to_list()[
				: dataframe.columns.get_loc(feature_name) + 1
			]
		...
		logger.warning(
			f"Feature {feature_name} was not found as a column in dataframe. No columns dropped."
		)
		return dataframe
...
def getOnlyPyradiomicsFeatures(radiomic_data: DataFrame) -> DataFrame:
	...
	if not diagnostic_data.empty:
		...
		pyradiomic_features_only = dropUpToFeature(
			radiomic_data, last_diagnostic_column, keep_feature_name_column=False
		)
		...
		if not original_feature_data.empty:
			...
			# Drop all the columns before the features start
			pyradiomic_features_only = dropUpToFeature(
				radiomic_data, first_pyradiomic_feature_column, keep_feature_name_column=True
			)
		else:
...
def getPatientIntersectionDataframes(
