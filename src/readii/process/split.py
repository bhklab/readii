...
def replaceColumnValues(
	dataframe: DataFrame, column_to_change: str, replacement_value_data: dict
) -> DataFrame:
	...
	for new_value, old_values in replacement_value_data.items():
		...
		values_not_found_in_column = set(old_values).difference(
			set(dataframe[column_to_change].unique())
		)
		...
		dataframe = dataframe.replace(to_replace=old_values, value=new_value)

	return dataframe


def splitDataByColumnValue(
	dataframe: DataFrame,
