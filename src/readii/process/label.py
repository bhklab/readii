...
def setPatientIdAsIndex(dataframe_to_index: DataFrame, patient_id_col: str = None) -> DataFrame:
...
def convertDaysToYears(
	dataframe_with_outcome: DataFrame, time_column_label: str, divide_by: int = 365
) -> DataFrame:
	...
	dataframe_with_outcome[years_column_label] = (
		dataframe_with_outcome[time_column_label] / divide_by
	)

	return dataframe_with_outcome


def timeOutcomeColumnSetup(
	dataframe_with_outcome: DataFrame,
	...
	if not np.issubdtype(dataframe_with_outcome[outcome_column_label].dtype, np.number):
		...
		if convert_to_years:
			...
			dataframe_with_standardized_outcome = convertDaysToYears(
				dataframe_with_standardized_outcome, outcome_column_label
			)

			# Rename the converted column with the standardized name
			dataframe_with_standardized_outcome.rename(
				columns={f"{outcome_column_label}_years": standard_column_label}, inplace=True
			)
		else:
...
def eventOutcomeColumnSetup(
	...
	if np.issubdtype(event_variable_type, np.number):
		dataframe_with_standardized_outcome[standard_column_label] = dataframe_with_outcome[
			outcome_column_label
		]
	...
	# Handle string event column
		...
		try:
			...
			dataframe_with_standardized_outcome[outcome_column_label] = dataframe_with_outcome[
				outcome_column_label
			].str.lower()
		...
		existing_event_values = sorted(
			dataframe_with_standardized_outcome[outcome_column_label].unique()
		)
		...
		if not event_column_value_mapping:
			...
			event_column_value_mapping = {
				status.lower(): value for status, value in event_column_value_mapping.items()
			}
		...
		replacement_value_data = dict(
			[num_event, str_event] for str_event, num_event in event_column_value_mapping.items()
		)

		# Replace the string event values in the standardized column with the numeric event values
		dataframe_with_standardized_outcome = replaceColumnValues(
			dataframe_with_standardized_outcome,
