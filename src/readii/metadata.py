...
def createImageMetadataFile(
    outputDir, parentDirPath, datasetName, segType, imageFileListPath, update=False
):
    ...
    if segType == 'RTSTRUCT':
        imageFileEdgesPath = os.path.join(
            parentDirPath + '/.imgtools/imgtools_' + datasetName + '_edges.csv'
        )
        getCTWithSegmentation(
            imgFileEdgesPath=imageFileEdgesPath, segType=segType, outputFilePath=imageMetadataPath
        )
    elif segType == 'SEG':
        matchCTtoSegmentation(
            imgFileListPath=imageFileListPath, segType=segType, outputFilePath=imageMetadataPath
        )
    else:
...
def saveDataframeCSV(dataframe: pd.DataFrame, outputFilePath: str) -> None:
...
def getSegmentationType(imgFileListPath: str) -> Literal['RTSTRUCT', 'SEG']:
